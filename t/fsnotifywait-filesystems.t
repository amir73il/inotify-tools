...
Verify that fsnotifywait --fanotify works correctly
...
if fanotify_supported --filesystem && is_root && btrfs_supported; then
    ...
    mount_btrfs_with_subvolumes 120M btrfs_check subvol1 || {
    ...
    if test -z "$test_skip_btrfs_fanotify"; then
        ...
        ../../src/fsnotifywait --fanotify --timeout 1 btrfs_check/subvol1/testdir 2>error.log || {
...
if fanotify_supported && overlayfs_supported; then
    ...
    if test -z "$test_skip_overlayfs"; then
        ../../src/fsnotifywait --fanotify --timeout 1 overlay_test_check_overlay 2>error.log || {
